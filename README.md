...
Instead of storing `.cln` source files on the filesystem, you can keep them in an H2 relational database. This is useful when source code lives in a managed store, is deployed as part of a service, or needs versioning and metadata in a structured way.

### How It Works

The interpreter detects that a JDBC URL is supplied (any value starting with `jdbc:`) for the `-cp` / `--cln_path` option **or** the `CLN_PATH` environment variable and automatically switches from the filesystem loader to the `H2Loader`. All other behavior (package resolution, import linking, execution) stays identical.

### Schema

The loader automatically creates the following table the first time it connects:
...
If you want to use a different JDBC-compatible database, place its driver JAR on the classpath and supply the driver class name via one of the options above.
...
```sql
-- Connect to your H2 database, then:
...
**In-memory H2 database** (useful for testing and ephemeral scenarios):
```bash
...
**Explicit driver class** (when using a non-H2 JDBC driver):
```bash
java -jar target/core-1.0-SNAPSHOT-fat.jar \
...
```bash
export CLN_DB_DRIVER=org.h2.Driver
export CLN_PATH=jdbc:h2:./mydb
java -jar target/core-1.0-SNAPSHOT-fat.jar -v myapp.main
```
...
2. **Seed the database** (using H2's built-in shell or any JDBC client):
   ...
   INSERT INTO CLN_SOURCE (package, source) VALUES
       ('hello', 'package hello;
import std.console.*;
(var int result = 0) main() { writeLine(\"Hello from H2!\"); return; }
');"
   ```
...
- **H2Loader**: Load and compile CLN source stored in an H2 (JDBC) database
